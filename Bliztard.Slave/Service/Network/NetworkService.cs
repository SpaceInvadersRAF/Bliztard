...
public class NetworkService(IHttpClientFactory httpClientFactory, ILogger<NetworkService> logger) : INetworkService
{
    ...
    public async Task<HttpResponseMessage> TwincateData(MachineInfo currMachineInfo, TwincateFileRequest twincateFile, MultipartFormDataContent content)
    {
        ...
        m_Logger.LogDebug("Timestamp: {Timestamp:HH:mm:ss.ffffff} | MachineId: {MachineId} | Resource: {Resource} | ReplicationId: {ReplicationId} | Twincate", DateTime.Now, currMachineInfo.Id,
                          twincateFile.Resource, twincateFile.MachineInfo.Id);
    ...
    public async Task<HttpResponseMessage> NotifyLogContent(MachineInfo currMachineInfo, NotifyLogContentRequest request, CancellationToken cancellationToken)
    {
        var httpClient = m_HttpClientFactory.CreateClient(Configuration.HttpClient.FileNotifyLogContent);

        m_Logger.LogDebug("Timestamp: {Timestamp:HH:mm:ss.ffffff} | MachineId: {MachineId} | Resources: {Resources} | Notify Log Content", DateTime.Now, currMachineInfo.Id,
                          string.Join(", ", request.SaveFileRequest.Select(fileInfo => fileInfo.Resource)));

        return await httpClient.PostAsJsonAsync(Configuration.Endpoint.Files.NotifyLogContent, request, cancellationToken);
    }
}
